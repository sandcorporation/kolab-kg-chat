...
서비스 DB엔 **임베딩·설명만** 담기고, 상품 사실(이름·가격·속성)은 채팅 때 소스에서 하이드레이션합니다(ADR-0016).
...
사용자가 한 번 질문하면, 아래 파이프라인이 돌며 SSE로 응답을 실시간 스트리밍합니다. 상품 사실(이름·가격·속성·이미지)은 서비스 DB에 복제하지 않고 **선택된 상품만 그 자리에서 소스 DB에서 하이드레이션**합니다(Route C, ADR-0016).
...
① QueryAnalyzer (gpt-4o-mini) — 질의 이해·정규화 (ADR-0017)
   ...
   │                                                                      │
   ▼(새 검색)                                                              │
② HybridRetriever — 서비스 DB(kg_embedding)에서 후보 검색 (ADR-0014/0015)     │
...
- **소스 하이드레이션(Route C)** — 서비스 DB엔 임베딩·설명·검색용 숫자 컬럼만 있고, 상품 사실은 채팅 때 소스에서 조회합니다. 그래서 **API가 적재와 동일한 `SOURCE_DB_*`를 봐야** 카드가 채워집니다.
