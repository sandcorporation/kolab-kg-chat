...
실험·연구 장비 쇼핑몰용 **적합성 추천 챗봇**입니다. 아래 순서대로 진행하시면 서비스를 띄우고 → 소량 데이터로 실제 동작을 확인하고 → 나머지를 전부 적재하고 → 변경분을 추적하는 워커까지 올리실 수 있습니다.
...
1. 소스 DB의 상품 행에 **직접-PDF URL 컬럼**을 두고 채웁니다(호스팅 권장). 컬럼명은 `PDF_FIELD`로 지정(기본 `it_pdf_url`).
