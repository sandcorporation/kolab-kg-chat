...
class FakeRetriever:
    ...
    async def retrieve(self, keywords, semantic_query, k=None):
        self.calls.append((keywords, semantic_query))
        return self._c


class FakeAnalyzer:
    """질의를 그대로 통과시키며 분석 입력을 기록한다."""
    def __init__(self):
        self.seen: list[str] = []
...
async def test_astream_selects_and_streams_rationale():
    ...
    events = [e async for e in RagRecommender(model, FakeRetriever(CANDS), FakeAnalyzer()).astream("유리 플라스크")]
...
async def test_selection_none_gives_empty_and_clarifies():
    ...
    events = [e async for e in RagRecommender(model, FakeRetriever(CANDS), FakeAnalyzer()).astream("모호")]
...
async def test_retrieval_uses_current_query_only():
    # 검색(질의 이해 포함)은 현재 질의로만 구동된다(히스토리 무관) — 앵커링 버그 근본 해소
    analyzer = FakeAnalyzer()
    model = ScriptedChatModel(responses=[AIMessage(content="선택: 없음\n\n네.")])
    _ = [e async for e in RagRecommender(model, FakeRetriever(CANDS), analyzer).astream(
        "어떤 상품 있어?",
        history=[{"role": "user", "content": "핀셋있어?"},
                 {"role": "assistant", "content": "핀셋 못 찾음"}],
    )]
    assert analyzer.seen == ["어떤 상품 있어?"]  # 히스토리 아닌 현재 질의만 분석
...
async def test_selection_without_rationale_still_speaks():
    ...
    model = ScriptedChatModel(responses=[AIMessage(content="선택: 1")])
    events = [e async for e in RagRecommender(model, FakeRetriever(CANDS), FakeAnalyzer()).astream("유리")]
...
async def test_malformed_output_is_safe():
    ...
    events = [e async for e in RagRecommender(model, FakeRetriever(CANDS), FakeAnalyzer()).astream("q")]
    result = [e for e in events if e["type"] == "result"][-1]
    assert result["recommended_ids"] == []
